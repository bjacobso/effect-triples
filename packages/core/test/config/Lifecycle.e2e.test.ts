...
describe("an employee's I-9, end to end", () => {
  it.effect("runs the whole stack over the real domain", () =>
    Effect.gen(function* () {
      ...
      const v1 = yield* releaseWithRules(InMemoryConfigStore.empty(), "2026.1", BASELINE);
      let store = yield* InMemoryConfigStore.setRef(v1.store, "live", v1.snapshot.id);
      ...
      step = Reactor.react(reactor, [Reactor.factKey(EE, "work_auth")], envOf());
      reactor = step.reactor;
      ...
      expect(step.flipped).toEqual([{ key: "overdue", from: "false", to: "true" }]);

      // -- 5. The employer completes section 2 -------------------------------
      world = World.withFact(world, EE, "i9_verified_at", now);
      step = Reactor.react(reactor, [Reactor.factKey(EE, "i9_verified_at")], envOf());
      reactor = step.reactor;

      expect([...step.flipped].sort((a, b) => a.key.localeCompare(b.key))).toEqual([
        // prettier-ignore
      ...
      const ssnRead = compliance.observed.find((o) => o._tag === "Fact" && o.attribute === "ssn")!;
      ...
      expect(
        Evaluate.replay(QUESTIONS.satisfied, compliance, world, clockAt(now), v1.catalog, EE),
      ).toBe(true);
      ...
      const changes = InMemoryConfigStore.changesBetween(v2.store, v1.snapshot, v2.snapshot);
      ...
      // -- 9. Publish, and react --------------------------------------------
      store = yield* InMemoryConfigStore.setRef(v2.store, "live", v2.snapshot.id);
  ...
  it.effect("keeps two employees' decisions independent", () =>
    Effect.gen(function* () {
      ...
      expect(Reactor.answer(reaction.reactor, "ee_2")).toBe(Reactor.answer(reactor, "ee_2"));
    }),
  );
  ...
  it.effect("ships the config and the rules that read it as one unit", () =>
    Effect.gen(function* () {
      ...
      const kinds = new Set([...ConfigNode.walk(v1.snapshot.root)].map(({ node }) => node.kind));
