...
import * as CanonicalJson from "../../src/content/CanonicalJson";
...
describe("ConfigNode identity", () => {
  ...
  it.effect("bubbles a leaf change all the way to the root", () =>
    Effect.gen(function* () {
      ...
      expect(before.children[0].node.cid).not.toEqual(after.children[0].node.cid);
    }),
  );
  ...
  it.effect("rejects two children sharing a key in one relation", () =>
    Effect.gen(function* () {
      ...
      expect(error._tag === "DuplicateChildKeyError" ? error.duplicate : undefined).toEqual(
        "field:alpha",
      );
    }),
  );
      ...
      const withText = yield* ConfigNode.closureId(subject, [
        {
          kind: "dep",
          key: "alpha",
          cid: ContentId.hash(ContentId.Domain.configNode, "scalar:text"),
        },
      ]);
      const withNumber = yield* ConfigNode.closureId(subject, [
        {
          kind: "dep",
          key: "alpha",
          cid: ContentId.hash(ContentId.Domain.configNode, "scalar:number"),
        },
      ]);
  ...
  it.effect("is insensitive to dependency order and duplication", () =>
    Effect.gen(function* () {
      ...
      const dep = (key: string) => ({
        kind: "dep" as const,
        key,
        cid: ContentId.hash(ContentId.Domain.configNode, key),
      });

      const one = yield* ConfigNode.closureId(subject, [dep("alpha"), dep("beta")]);
      const two = yield* ConfigNode.closureId(subject, [dep("beta"), dep("alpha"), dep("beta")]);

      expect(one).toEqual(two);
    }),
  );

  it.effect("fails loudly when one dependency arrives with two content ids", () =>
    Effect.gen(function* () {
      const subject = yield* ConfigNode.make({ kind: "tree", key: "root" });
      const error = yield* ConfigNode.closureId(subject, [
        {
          kind: "dep",
          key: "alpha",
          cid: ContentId.hash(ContentId.Domain.configNode, "one"),
        },
        {
          kind: "dep",
          key: "alpha",
          cid: ContentId.hash(ContentId.Domain.configNode, "two"),
        },
      ]).pipe(Effect.flip);
...
describe("ConfigNode.diff", () => {
  ...
  it.effect("reports only the path that changed", () =>
    Effect.gen(function* () {
      const untouched = yield* page("untouched", [yield* leaf("a"), yield* leaf("b")]);
  ...
  it.effect("distinguishes add, remove and reorder", () =>
    Effect.gen(function* () {
      ...
      const changes = ConfigNode.diff(yield* page("p", [a, b]), yield* page("p", [c, b]));
      ...
      const reorder = ConfigNode.diff(yield* page("p", [a, b]), yield* page("p", [b, a]));
...
describe("ConfigNode.makeTyped", () => {
  ...
  it.effect("keeps the schema out of cid but inside stamp", () =>
    Effect.gen(function* () {
      ...
      expect(after.type && T.id(after.type)).not.toEqual(before.type && T.id(before.type));
      expect(yield* ConfigNode.stamp(after)).not.toEqual(yield* ConfigNode.stamp(before));
    }),
  );
  ...
  it.effect("stamps untyped nodes distinctly from typed ones", () =>
    Effect.gen(function* () {
      ...
      expect(yield* ConfigNode.stamp(typed)).not.toEqual(yield* ConfigNode.stamp(untyped));
    }),
  );
});
