...
describe("a rule is a config node", () => {
  it.effect("derives its refs from the expression, not from the author", () =>
    Effect.gen(function* () {
      ...
      expect(node.refs).toEqual([{ rel: "calls", kind: "rule", key: "is-caregiver" }]);
    }),
  );
...
describe("config and facts invalidate through one mechanism", () => {
  ...
  it.effect("observes a resolved rule by content id, like a fact by value", () =>
    Effect.gen(function* () {
...
describe("pinning: two versions of a rule cannot be confused", () => {
  ...
  it.effect("puts the rule's version inside the evaluation's own identity", () =>
    Effect.gen(function* () {
...
describe("rules that call rules", () => {
  it.effect("composes, and observes every rule on the path", () =>
    Effect.gen(function* () {
      ...
      expect(configDeps.sort()).toEqual(["eligible", "is-caregiver", "started"]);
    }),
  );
  ...
  it.effect("refuses to loop on a recursive rule", () =>
    Effect.gen(function* () {
      ...
      expect([...Evaluate.walk(evaluation)].some((n) => n.reason?.includes("recursive"))).toBe(
        true,
      );
    }),
  );
