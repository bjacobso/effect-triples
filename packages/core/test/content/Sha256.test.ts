...
import * as Sha256 from "../../src/content/Sha256";

/** The oracle. Tests run on Node, so the reference implementation is available. */
const reference = (input: string) => createHash("sha256").update(input, "utf8").digest("hex");
...
describe("Sha256", () => {
  ...
  it("agrees over random inputs", () => {
    ...
    for (let i = 0; i < 300; i++) {
      ...
      expect(Sha256.hex(input), JSON.stringify(input)).toEqual(reference(input));
    }
  });
