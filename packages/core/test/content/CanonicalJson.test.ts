...
describe("CanonicalJson", () => {
  ...
  it.effect("drops undefined properties but keeps undefined array slots", () =>
    Effect.gen(function* () {
      ...
      expect(yield* CanonicalJson.encode([1, undefined, 2])).toEqual("[1,null,2]");
    }),
  );
  ...
  it.effect("rejects values with no stable encoding", () =>
    Effect.gen(function* () {
      const reasons = yield* Effect.all(
        [{ deep: { bad: Number.NaN } }, { at: new Date(0) }, { fn: (() => 1) as never }].map(
          (value) =>
  ...
  it.effect("rejects cycles rather than looping forever", () =>
    Effect.gen(function* () {
      ...
      const error = yield* CanonicalJson.encode(cyclic as CanonicalJson.CanonicalValue).pipe(
        Effect.flip,
      );
