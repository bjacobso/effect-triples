...
export const factKey = (entity: string, attribute: string): string => `${entity}/${attribute}`;

export const make = (facts: Readonly<Record<string, Value>>): World => ({
  facts: new Map(Object.entries(facts)),
});

export const withFact = (world: World, entity: string, attribute: string, value: Value): World => {
  const facts = new Map(world.facts);
  facts.set(factKey(entity, attribute), value);
  return { facts };
};

export const withoutFact = (world: World, entity: string, attribute: string): World => {
  const facts = new Map(world.facts);
  facts.delete(factKey(entity, attribute));
  return { facts };
};

export const read = (world: World, entity: string, attribute: string): Value | undefined =>
  world.facts.get(factKey(entity, attribute));
 ...
 */
export const closureId = (observed: ReadonlyArray<Observed>): ContentId.ContentId => {
  ...
  return ContentId.hash(ContentId.Domain.observationClosure, CanonicalJson.encodeOrThrow(sorted));
};

const VALUE_DOMAIN = ContentId.Domain.observationValue;
...
export const observe = (world: World, entity: string, attribute: string): Observed => {
...
export const matches = (observed: Observed, value: Value): boolean =>
  observed._tag === "Fact" && observed.present && observed.digest === valueDigest(value);
