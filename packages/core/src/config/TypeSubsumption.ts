...
export const isCompatible = (verdict: Verdict): boolean => verdict._tag !== "Narrows";

const widens = (...reasons: ReadonlyArray<string>): Verdict => Verdict.Widens({ reasons });
const narrows = (...reasons: ReadonlyArray<string>): Verdict => Verdict.Narrows({ reasons });
...
const unwrap = (expr: TypeExpr.TypeExpr): { base: TypeExpr.TypeExpr; bounds: Bounds } =>
  expr._tag === "Constrained"
    ? { base: expr.base, bounds: boundsOf(expr.constraints) }
    : { base: expr, bounds: {} };

const go = (from: TypeExpr.TypeExpr, to: TypeExpr.TypeExpr, path: string): Verdict => {
  ...
  if (t.base._tag === "Union") {
    const accepted = t.base.members.some((m) => isCompatible(go(f.base, m, at)));
    return accepted
  ...
  const structural = ((): Verdict => {
    ...
    switch (f.base._tag) {
      ...
      case "Ref": {
        ...
        return narrows(`${at} points at ${(t.base as TypeExpr.Ref).kind}, not ${f.base.kind}`);
      }
      case "List":
 ...
 */
export const subsumes = (from: TypeExpr.TypeExpr, to: TypeExpr.TypeExpr): Verdict =>
  go(from, to, "");
