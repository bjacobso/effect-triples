...
const NODE_DOMAIN = ContentId.Domain.configNode;
const CLOSURE_DOMAIN = ContentId.Domain.configClosure;
const STAMP_DOMAIN = ContentId.Domain.configStamp;
...
export class DuplicateChildKeyError extends Data.TaggedError("DuplicateChildKeyError")<{
  readonly kind: string;
  readonly key: string;
...
export class ConflictingClosureEntryError extends Data.TaggedError("ConflictingClosureEntryError")<{
  readonly kind: string;
  readonly key: string;
  readonly message: string;
}> {}
...
const sortRefs = (refs: ReadonlyArray<ConfigRef>): ReadonlyArray<ConfigRef> =>
  [...refs]
    .sort((a, b) => cmp(a.rel, b.rel) || cmp(a.kind, b.kind) || cmp(a.key, b.key))
    ...
    .filter((ref, index, all) => {
      ...
      return (
        previous === undefined ||
        ref.rel !== previous.rel ||
        ref.kind !== previous.kind ||
        ref.key !== previous.key
      );
    });

const refsValue = (refs: ReadonlyArray<ConfigRef>): CanonicalJson.CanonicalValue =>
...
export const make = (input: {
  ...
  readonly children?: ReadonlyArray<ConfigChild>;
  readonly refs?: ReadonlyArray<ConfigRef>;
}): Effect.Effect<ConfigNode, DuplicateChildKeyError | CanonicalJson.CanonicalEncodingError> =>
  Effect.gen(function* () {
...
export const makeTyped = (input: {
  ...
  ConfigNode,
  DuplicateChildKeyError | CanonicalJson.CanonicalEncodingError | Schema.SchemaError
> =>
  Effect.gen(function* () {
 ...
 */
export const typeIds = (node: ConfigNode): ReadonlyArray<ContentId.ContentId> => {
...
> =>
  Effect.gen(function* () {
    ...
    const sorted = [...byKey.values()].sort((a, b) => cmp(a.kind, b.kind) || cmp(a.key, b.key));
 ...
 */
export const flatten = (node: ConfigNode): ReadonlyMap<ContentId.ContentId, ConfigNode> => {
...
export const diff = (
  ...
  const beforeChildren = new Map(
    before.children.map((child, index) => [`${child.rel} ${child.node.key}`, { child, index }]),
  );
  const afterChildren = new Map(
    after.children.map((child, index) => [`${child.rel} ${child.node.key}`, { child, index }]),
  );
  ...
  changes.unshift({
    ...
    refsChanged: encodedOf(refsValue(before.refs)) !== encodedOf(refsValue(after.refs)),
    reordered,
  });
