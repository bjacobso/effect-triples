...
export interface Entity<K extends string, I, C extends ChildFields, R extends RefFields> {
    ...
    node: (input) =>
      Effect.gen(function* () {
        ...
        const nested = Object.entries(childFields).flatMap(([rel]) =>
          ((input.children?.[rel] ?? []) as ReadonlyArray<ConfigNode.ConfigNode>).map((node) => ({
            rel,
            node,
          })),
        );
 ...
 */
export const withType = <K extends string, I, C extends ChildFields, R extends RefFields>(
