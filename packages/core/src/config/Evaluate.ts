...
export const env = (
  ...
  subject?: string,
): Env => ({ world, clock, catalog, ...(subject !== undefined && { subject }) });
...
const dedupe = (observed: ReadonlyArray<World.Observed>): ReadonlyArray<World.Observed> => {
...
const notOf = (t: Truth): Truth => (t === "unknown" ? "unknown" : t === "true" ? "false" : "true");
...
const go = (expr: BoolExpr.BoolExpr, env: Env, calling: ReadonlySet<string>): Evaluation => {
...
export const impact = (
  ...
  subjects.forEach((world, subject) => {
    ...
    const configDeps = previous.evaluation.observed.filter((o) => o._tag === "Config");
...
export const verify = (evaluation: Evaluation, path = "root"): ReadonlyArray<Tampered> => {
  ...
  return actual === evaluation.cid ? found : [...found, { path, claimed: evaluation.cid, actual }];
};
...
export const replay = (
  ...
  catalog: Catalog.Catalog = Catalog.empty,
  subject?: string,
): boolean => go(expr, env(world, clock, catalog, subject), new Set()).cid === evaluation.cid;
