...
const fixtures = {
  ...
  "entity-only": (n) =>
    [
      `import * as Entity from "../../../src/config/Entity";`,
      `import * as T from "../../../src/config/TypeExpr";`,
      INPUT,
      `const AttrKind = Entity.kind("attribute");`,
      `const Shared = T.struct({ slug: T.required(T.text) });`,
      ...Array.from({ length: n }, (_, i) => `const Kind${i} = Entity.kind("entity${i}");`),
  ...
  combined: (n) =>
    [
      ...
      `const AttrKind = Entity.kind("attribute");`,
      ...Array.from({ length: n }, (_, i) => `const Kind${i} = Entity.kind("entity${i}");`),
      ...Array.from({ length: n }, (_, i) => `${typeAttrs(i)}\n${entity(i, n, `Attrs${i}`)}`),
    ].join("\n"),
};
...
const time = (include) => {
  ...
  try {
    execFileSync("pnpm", ["exec", "tsc", "-p", join(DIR, "tsconfig.json"), "--noEmit"], {
      stdio: "pipe",
    });
  } catch (error) {
...
try {
  ...
  console.log(`baseline (src only)      ${baseline.toFixed(0).padStart(6)} ms\n`);
  ...
  for (const n of SIZES) {
    ...
    for (const [name, make] of Object.entries(fixtures)) {
      ...
      console.log(`  ${name.padEnd(22)} +${marginal[name].toFixed(0).padStart(5)} ms`);
    }
    // The two things `combined` is actually made of.
    const parts = marginal["type-only"] + marginal["entity-only"];
    console.log(`  ${"sum of parts".padEnd(22)} +${parts.toFixed(0).padStart(5)} ms`);
    console.log(`  interaction factor      ${(marginal.combined / parts).toFixed(1)}x\n`);
  }
} finally {
