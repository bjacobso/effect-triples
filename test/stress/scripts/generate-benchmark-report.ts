 ...
 *
 * Runs the stress test against SQLite, PostgreSQL (via Docker), and the
 * in-memory KV store, collects structured JSON results, and generates a
 * markdown performance report at docs/performance.md.
 *
 * Uses the repo-level docker-compose.test.yml (same as integration tests)
 * with dynamic port discovery to avoid port conflicts.
 *
 * Usage:
...
function cleanupJsonFiles() {
  for (const backend of ["sqlite", "pg", "kv"]) {
...
function generateMarkdown(results: BenchmarkResults[]): string {
  ...
  w("");
  w(
    `> Auto-generated on ${new Date().toISOString().split("T")[0]} by \`pnpm --filter @open-ontology/core-stress benchmark\`.`,
  );
  ...
  w("");
  w("Datalog queries compiled to SQL (SQLite/PG) or executed natively against the hexastore (KV).");
  w("");
  ...
  w(
    "- SQLite and PG compile Datalog to SQL. KV executes Datalog natively against the hexastore. Results are deterministic for the same dataset.",
  );
  w(
  ...
  w("");
  w("```bash");
  w("# Full benchmark (1M employees, 10 update rounds, SQLite + PostgreSQL)");
  w("pnpm --filter @open-ontology/core-stress benchmark");
  w("");
  w("# Customize scale");
  w(
  ...
  w("STRESS_UPDATE_ROUNDS=0 pnpm --filter @open-ontology/core-stress benchmark");
  w("");
  w("# Skip PostgreSQL (SQLite + KV only, no Docker required)");
  w("BENCHMARK_SKIP_PG=true pnpm --filter @open-ontology/core-stress benchmark");
  w("");
  w("# Skip KV (SQLite + PostgreSQL only)");
  w("BENCHMARK_SKIP_KV=true pnpm --filter @open-ontology/core-stress benchmark");
  w("```");
  w("");
...
async function main() {
  ...
  if (skipPg && skipSqlite && skipKv) {
    fail("All backends are skipped. Nothing to benchmark.");
  }

  // Check Docker if we need PG
  if (!skipPg && !dockerAvailable()) {
    warn("Docker is not available. Skipping PostgreSQL benchmark.");
    warn("Install Docker or set BENCHMARK_SKIP_PG=true to silence this warning.");
  ...
  try {
    ...
    if (needsPg && pgUrl) {
      runBenchmark("pg", { DATABASE_URL: pgUrl });
    }

    // Run KV benchmark
    // The KV backend has no SqlClient, so updates must be skipped (STRESS_UPDATE_ROUNDS=0).
    if (!skipKv) {
      runBenchmark("kv", { STRESS_UPDATE_ROUNDS: "0" });
    }
