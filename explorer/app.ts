...
import * as ConfigNode from "../src/ConfigNode";
...
import {
  ...
  ruleSet,
  type AccountConfig,
} from "../src/domain/OnboardingConfig";
...
type State = {
  ...
  cursor: number;
  last?: { considered: ReadonlyArray<string>; flipped: ReadonlyArray<Reactor.Flip> }; // prettier-ignore
  tampered: boolean;
  published: boolean;
};
...
const renderRelease = () => {
  ...
  note.replaceChildren(
    document.createTextNode("Snapshot id "),
...
$("step").addEventListener("click", () => {
  ...
  const changed = steps[s.cursor].apply();
...
$("verify").addEventListener("click", () => {
  ...
  if (problems.length === 0) {
    ...
    out.append(el("b", undefined, `Tampered at ${problems[0].path}.`));
    out.append(
      document.createTextNode(
        ` Claimed ${short(problems[0].claimed)}, recomputes to ${short(problems[0].actual)}. The edit breaks the root, so it cannot be passed off as the original.`
      )
    );
  }
});
...
$("propose").addEventListener("click", () => {
  ...
  const changes = ConfigStore.changesBetween(v2.store, s.v1.snapshot, v2.snapshot); // prettier-ignore
