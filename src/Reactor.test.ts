...
const eligible = (ee: string) =>
  B.all([B.rule("is-caregiver-" + ee), B.exists(ee, "work_auth")]);

const envOf = (
  world: World.World,
  catalog: Catalog.Catalog,
  now = MARCH_3
): Evaluate.Env => Evaluate.env(world, { now, granularity: "day" }, catalog);
...
describe("the reverse index finds only what depended on a change", () => {
  it.effect("ignores registrations that never read the changed fact", () =>
    Effect.gen(function* () {
      ...
      expect(Reactor.affected(r, [Reactor.factKey("a", "role")])).toEqual([
        "a",
      ]);
      expect(Reactor.affected(r, [Reactor.factKey("b", "role")])).toEqual([
        "b",
      ]);
...
describe("recomputation is not the same event as change", () => {
  it.effect("reports flips separately from what it reconsidered", () =>
    Effect.gen(function* () {
      ...
      const reaction = Reactor.react(
        r,
        [Reactor.ruleKey("is-caregiver-a")],
        envOf(world, after)
      );
...
describe("cost tracks the change, not the population", () => {
  it.effect("touches one of a thousand registrations", () =>
    Effect.gen(function* () {
      ...
      const reaction = Reactor.react(
        r,
        [Reactor.factKey("ee500", "role")],
        envOf(world, catalog)
      );

      expect(reaction.considered).toEqual(["ee500"]);
      expect(reaction.flipped).toEqual([
        { key: "ee500", from: "true", to: "false" },
      ]);

      // The other 999 answers are untouched objects, not recomputed ones.
      expect(Reactor.answer(reaction.reactor, "ee499")).toBe(
        Reactor.answer(r, "ee499")
      );
    })
  );
    ...
    Effect.gen(function* () {
      ...
      for (let step = 0; step < 300; step++) {
        ...
        for (const [key, reg] of r.registrations) {
          ...
          expect(reg.evaluation.cid, `${key} @ step ${step}`).toEqual(
            fresh.cid
          );
        }
      }
